...
def train(model,
    ...
    return model,epoch_time, epoch_loss, epoch_accuracy
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            logits, probs = model(premises,
                                  ...
                                  hypotheses,
                                  hypotheses_lengths)
            y_pred.extend(np.array(probs.cpu())[:,1])   
            y_true.extend(list(labels.cpu().numpy())) 
    epoch_time = time.time() - epoch_start
    auc = metrics.roc_auc_score(y_true,y_pred,average='macro')
    y_pred=list(map(lambda x:1 if x>0.85 else 0,y_pred))
    acc=metrics.accuracy_score(y_true,y_pred)
    return epoch_time,auc,acc
